...
from loans l
inner join bus_analytics.payday_loan_chains_performance blp on blp.base_loan_id = l.id
...
and blp.loan_chain_due_date <= current_date - interval '90 days'
...
--may want to not include this in the data pull or remove it when processing in R/sas
...
,(select avg(tb.initial_default_flg) from mdalgleish.payday_bad_rates tb where tb.state = td2.state and tb.due_date_adjusted between td2.approval_processed - interval '33 days' and td2.approval_processed - interval '3 days' and tb.existing_flg = 'f') as state_def_rate_30_day

,(select avg(tb.initial_default_flg) from mdalgleish.payday_bad_rates tb where tb.lead_provider = td2.lead_provider and tb.due_date_adjusted between td2.approval_processed - interval '33 days' and td2.approval_processed - interval '3 days' and tb.existing_flg = 'f') as lead_def_rate_30_day

,(select avg(tb.initial_default_flg) from mdalgleish.payday_bad_rates tb where tb.rv_score_tier = (case when td2.rv_score >= 600.0 then 1 when td2.rv_score >= 578.0 then 2 when td2.rv_score >= 556.0 then 3 else 4 end) and tb.due_date_adjusted between td2.approval_processed - interval '33 days' and td2.approval_processed - interval '3 days' and tb.existing_flg = 'f') as rv_score_def_rate_30_day

,(select avg(tb.extended_flg) from mdalgleish.payday_bad_rates tb where tb.state = td2.state and tb.due_date_adjusted between td2.approval_processed - interval '33 days' and td2.approval_processed - interval '3 days' and tb.existing_flg = 'f') as state_ext_rate_30_day

,(select avg(tb.extended_flg) from mdalgleish.payday_bad_rates tb where tb.lead_provider = td2.lead_provider and tb.due_date_adjusted between td2.approval_processed - interval '33 days' and td2.approval_processed - interval '3 days' and tb.existing_flg = 'f') as lead_ext_rate_30_day

,(select avg(tb.extended_flg) from mdalgleish.payday_bad_rates tb where tb.rv_score_tier = (case when td2.rv_score >= 600.0 then 1 when td2.rv_score >= 578.0 then 2 when td2.rv_score >= 556.0 then 3 else 4 end) and tb.due_date_adjusted between td2.approval_processed - interval '33 days' and td2.approval_processed - interval '3 days' and tb.existing_flg = 'f') as rv_score_ext_rate_30_day

,(select avg(tb.initial_default_flg) from mdalgleish.payday_bad_rates tb where tb.state = td2.state and tb.due_date_adjusted between td2.approval_processed - interval '63 days' and td2.approval_processed - interval '3 days' and tb.existing_flg = 'f') as state_def_rate_60_day

,(select avg(tb.initial_default_flg) from mdalgleish.payday_bad_rates tb where tb.lead_provider = td2.lead_provider and tb.due_date_adjusted between td2.approval_processed - interval '63 days' and td2.approval_processed - interval '3 days' and tb.existing_flg = 'f') as lead_def_rate_60_day

,(select avg(tb.initial_default_flg) from mdalgleish.payday_bad_rates tb where tb.rv_score_tier = (case when td2.rv_score >= 600.0 then 1 when td2.rv_score >= 578.0 then 2 when td2.rv_score >= 556.0 then 3 else 4 end) and tb.due_date_adjusted between td2.approval_processed - interval '63 days' and td2.approval_processed - interval '3 days' and tb.existing_flg = 'f') as rv_score_def_rate_60_day

,(select avg(tb.extended_flg) from mdalgleish.payday_bad_rates tb where tb.state = td2.state and tb.due_date_adjusted between td2.approval_processed - interval '63 days' and td2.approval_processed - interval '3 days' and tb.existing_flg = 'f') as state_ext_rate_60_day

,(select avg(tb.extended_flg) from mdalgleish.payday_bad_rates tb where tb.lead_provider = td2.lead_provider and tb.due_date_adjusted between td2.approval_processed - interval '63 days' and td2.approval_processed - interval '3 days' and tb.existing_flg = 'f') as lead_ext_rate_60_day

,(select avg(tb.extended_flg) from mdalgleish.payday_bad_rates tb where tb.rv_score_tier = (case when td2.rv_score >= 600.0 then 1 when td2.rv_score >= 578.0 then 2 when td2.rv_score >= 556.0 then 3 else 4 end) and tb.due_date_adjusted between td2.approval_processed - interval '63 days' and td2.approval_processed - interval '3 days' and tb.existing_flg = 'f') as rv_score_ext_rate_60_day

,(select avg(tb.initial_default_flg) from mdalgleish.payday_bad_rates tb where tb.state = td2.state and tb.due_date_adjusted between td2.approval_processed - interval '93 days' and td2.approval_processed - interval '3 days' and tb.existing_flg = 'f') as state_def_rate_90_day

,(select avg(tb.initial_default_flg) from mdalgleish.payday_bad_rates tb where tb.lead_provider = td2.lead_provider and tb.due_date_adjusted between td2.approval_processed - interval '93 days' and td2.approval_processed - interval '3 days' and tb.existing_flg = 'f') as lead_def_rate_90_day

,(select avg(tb.initial_default_flg) from mdalgleish.payday_bad_rates tb where tb.rv_score_tier = (case when td2.rv_score >= 600.0 then 1 when td2.rv_score >= 578.0 then 2 when td2.rv_score >= 556.0 then 3 else 4 end) and tb.due_date_adjusted between td2.approval_processed - interval '93 days' and td2.approval_processed - interval '3 days' and tb.existing_flg = 'f') as rv_score_def_rate_90_day

,(select avg(tb.extended_flg) from mdalgleish.payday_bad_rates tb where tb.state = td2.state and tb.due_date_adjusted between td2.approval_processed - interval '93 days' and td2.approval_processed - interval '3 days' and tb.existing_flg = 'f') as state_ext_rate_90_day

,(select avg(tb.extended_flg) from mdalgleish.payday_bad_rates tb where tb.lead_provider = td2.lead_provider and tb.due_date_adjusted between td2.approval_processed - interval '93 days' and td2.approval_processed - interval '3 days' and tb.existing_flg = 'f') as lead_ext_rate_90_day

,(select avg(tb.extended_flg) from mdalgleish.payday_bad_rates tb where tb.rv_score_tier = (case when td2.rv_score >= 600.0 then 1 when td2.rv_score >= 578.0 then 2 when td2.rv_score >= 556.0 then 3 else 4 end) and tb.due_date_adjusted between td2.approval_processed - interval '93 days' and td2.approval_processed - interval '3 days' and tb.existing_flg = 'f') as rv_score_ext_rate_90_day
...
,round((nice_div(tbr.loan_amount::numeric,tbr.approval_amount::numeric)),5) as utilization_ratio
,blcp.loan_duration as actual_loan_duration_days
,blcp.number_funded_extensions as extension_count
,blp.initial_default_flg as base_loan_initial_default_flg
,blp.final_default_flg as base_loan_final_default_flg
,blcp.count_initial_default as loan_chain_initial_default_count
,case when blcp.count_final_default > 0 then 1 else 0 end as loan_chain_final_default_flg

,blcp.net_fees as loan_chain_net_fees

,nice_div(blcp.net_fees,tbr.loan_amount) as profit_rate

,case when tbr.funding_date_actual <= current_date - interval '6 months' then (select net_fees from mdalgleish.all_customer_net_fees where cnu_customer_id = tbr.customer_id and month_calculated = 3) else null end as three_month_net_fees
...
,case when tbr.funding_date_actual <= current_date - interval '6 months' then (select count(*) from loans where customer_id = tbr.customer_id and id > tbr.loan_id and funding_date_actual <= tbr.funding_date_actual + interval '6 months' and status_cd not in ('declined','withdrawn','on_hold','approved','applied')) else null end as additional_loan_objects_six_months

,case when tbr.funding_date_actual <= current_date - interval '12 months' then (select count(*) from loans where customer_id = tbr.customer_id and id > tbr.loan_id and funding_date_actual <= tbr.funding_date_actual + interval '12 months' and status_cd not in ('declined','withdrawn','on_hold','approved','applied')) else null end as additional_loan_objects_twelve_months

,case when tbr.funding_date_actual <= current_date - interval '18 months' then (select count(*) from loans where customer_id = tbr.customer_id and id > tbr.loan_id and funding_date_actual <= tbr.funding_date_actual + interval '18 months' and status_cd not in ('declined','withdrawn','on_hold','approved','applied')) else null end as additional_loan_objects_eighteen_months

into temp temp_performance
from temp_bad_rates tbr

left join bus_analytics.payday_loan_chains_performance blcp on blcp.base_loan_id = tbr.loan_id
left join bus_analytics.payday_loans_performance blp on blp.loan_id = tbr.loan_id
