...
and (select min(due_date) from oec.oec_statements where loan_id = l.id)::date <= current_date - interval '90 days'
...
-- probably don't include these people in the final data pull
...
,(select avg(tb.first_statement_initial_default_flag) from mdalgleish.loc_bad_rates tb where tb.state = td2.state and tb.first_statement_due_date between td2.approval_processed - interval '33 days' and td2.approval_processed - interval '3 days' and tb.existing_flg = 'f') as state_def_rate_30_day

,(select avg(tb.first_statement_initial_default_flag) from mdalgleish.loc_bad_rates tb where tb.lead_provider = td2.lead_provider and tb.first_statement_due_date between td2.approval_processed - interval '33 days' and td2.approval_processed - interval '3 days' and tb.existing_flg = 'f') as lead_def_rate_30_day

,(select avg(tb.first_statement_initial_default_flag) from mdalgleish.loc_bad_rates tb where tb.rv_score_tier = (case when td2.rv_score >= 600.0 then 1 when td2.rv_score >= 578.0 then 2 when td2.rv_score >= 556.0 then 3 else 4 end) and tb.first_statement_due_date between td2.approval_processed - interval '33 days' and td2.approval_processed - interval '3 days' and tb.existing_flg = 'f') as rv_score_def_rate_30_day

,(select avg(tb.first_statement_initial_default_flag) from mdalgleish.loc_bad_rates tb where tb.state = td2.state and tb.first_statement_due_date between td2.approval_processed - interval '63 days' and td2.approval_processed - interval '3 days' and tb.existing_flg = 'f') as state_def_rate_60_day

,(select avg(tb.first_statement_initial_default_flag) from mdalgleish.loc_bad_rates tb where tb.lead_provider = td2.lead_provider and tb.first_statement_due_date between td2.approval_processed - interval '63 days' and td2.approval_processed - interval '3 days' and tb.existing_flg = 'f') as lead_def_rate_60_day

,(select avg(tb.first_statement_initial_default_flag) from mdalgleish.loc_bad_rates tb where tb.rv_score_tier = (case when td2.rv_score >= 600.0 then 1 when td2.rv_score >= 578.0 then 2 when td2.rv_score >= 556.0 then 3 else 4 end) and tb.first_statement_due_date between td2.approval_processed - interval '63 days' and td2.approval_processed - interval '3 days' and tb.existing_flg = 'f') as rv_score_def_rate_60_day

,(select avg(tb.first_statement_initial_default_flag) from mdalgleish.loc_bad_rates tb where tb.state = td2.state and tb.first_statement_due_date between td2.approval_processed - interval '93 days' and td2.approval_processed - interval '3 days' and tb.existing_flg = 'f') as state_def_rate_90_day

,(select avg(tb.first_statement_initial_default_flag) from mdalgleish.loc_bad_rates tb where tb.lead_provider = td2.lead_provider and tb.first_statement_due_date between td2.approval_processed - interval '93 days' and td2.approval_processed - interval '3 days' and tb.existing_flg = 'f') as lead_def_rate_90_day

,(select avg(tb.first_statement_initial_default_flag) from mdalgleish.loc_bad_rates tb where tb.rv_score_tier = (case when td2.rv_score >= 600.0 then 1 when td2.rv_score >= 578.0 then 2 when td2.rv_score >= 556.0 then 3 else 4 end) and tb.first_statement_due_date between td2.approval_processed - interval '93 days' and td2.approval_processed - interval '3 days' and tb.existing_flg = 'f') as rv_score_def_rate_90_day

--,(select avg(tb.early_payoff_flg) from mdalgleish.loc_bad_rates tb where tb.state = td2.state and tb.first_statement_due_date between td2.approval_processed - interval '33 days' and td2.approval_processed - interval '3 days' and tb.existing_flg = 'f') as state_early_payoff_rate_30_day

--,(select avg(tb.early_payoff_flg) from mdalgleish.loc_bad_rates tb where tb.lead_provider = td2.lead_provider and tb.first_statement_due_date between td2.approval_processed - interval '33 days' and td2.approval_processed - interval '3 days' and tb.existing_flg = 'f') as lead_early_payoff_rate_30_day

--,(select avg(tb.early_payoff_flg) from mdalgleish.loc_bad_rates tb where tb.rv_score_tier = (case when td2.rv_score >= 600.0 then 1 when td2.rv_score >= 578.0 then 2 when td2.rv_score >= 556.0 then 3 else 4 end) and tb.first_statement_due_date between td2.approval_processed - interval '33 days' and td2.approval_processed - interval '3 days' and tb.existing_flg = 'f') as rv_score_early_payoff_rate_30_day

--,(select avg(tb.early_payoff_flg) from mdalgleish.loc_bad_rates tb where tb.state = td2.state and tb.first_statement_due_date between td2.approval_processed - interval '63 days' and td2.approval_processed - interval '3 days' and tb.existing_flg = 'f') as state_early_payoff_rate_60_day

--,(select avg(tb.early_payoff_flg) from mdalgleish.loc_bad_rates tb where tb.lead_provider = td2.lead_provider and tb.first_statement_due_date between td2.approval_processed - interval '63 days' and td2.approval_processed - interval '3 days' and tb.existing_flg = 'f') as lead_early_payoff_rate_60_day

--,(select avg(tb.early_payoff_flg) from mdalgleish.loc_bad_rates tb where tb.rv_score_tier = (case when td2.rv_score >= 600.0 then 1 when td2.rv_score >= 578.0 then 2 when td2.rv_score >= 556.0 then 3 else 4 end) and tb.first_statement_due_date between td2.approval_processed - interval '63 days' and td2.approval_processed - interval '3 days' and tb.existing_flg = 'f') as rv_score_early_payoff_rate_60_day
...
drop table if exists temp_loc_performance;
create temp table temp_loc_performance as (
select
td.loan_id
, case when s.total_interest_due + s.principal_due+ s.fees_due >0
			and coalesce (s.due_date_adjusted, s.due_date)<current_date
				then 1 else 0 end as positive_amount_due_flag
,case when exists (select ptc.id from payment_transactions_committed ptc
						where ptc.loan_id=l.id
						and ptc.eff_date BETWEEN COALESCE(s.due_date_adjusted, s.due_date)
						AND add_business_days(COALESCE(s.due_date_adjusted, s.due_date),5)
			...
			) then 1 else 0 end as statement_initial_default_flag
from temp_bad_rates td
inner join loans l on l.id = td.loan_id
left join oec.oec_statements s on l.id=s.loan_id
where not exists (select id from oec.oec_statements where loan_id=l.id and id<s.id));

drop table if exists temp_withdraw_numbers;
create temp table temp_withdraw_numbers as (
select td.loan_id,
sum(case when sub_type_cd IS NULL then 0 when sub_type_cd != 'principal_out' then 0 when status_cd IN ('returned','cancelled') then 0 when eff_date BETWEEN funding_date_actual and funding_date_actual + '3 months'::interval then -1*total_amount else 0 end) as total_withdrawn_3m,
sum(case when sub_type_cd IS NULL then 0 when sub_type_cd != 'principal_out' then 0 when status_cd IN ('returned','cancelled') then 0 when eff_date BETWEEN funding_date_actual and funding_date_actual + '6 months'::interval then -1*total_amount else 0 end) as total_withdrawn_6m,
sum(case when sub_type_cd IS NULL then 0 when sub_type_cd != 'principal_out' then 0 when status_cd IN ('returned','cancelled') then 0 when eff_date BETWEEN funding_date_actual and funding_date_actual + '12 months'::interval then -1*total_amount else 0 end) as total_withdrawn_12m,
sum(case when sub_type_cd IS NULL then 0 when sub_type_cd != 'principal_out' then 0 when status_cd IN ('returned','cancelled') then 0 else -1*total_amount end ) as total_withdrawn,
sum(case when sub_type_cd IS NULL then 0 when sub_type_cd != 'principal_out' then 0 when status_cd IN ('returned','cancelled') then 0 when eff_date BETWEEN funding_date_actual and funding_date_actual + '3 months'::interval then 1 else 0 end) as total_withdraws_3m,
sum(case when sub_type_cd IS NULL then 0 when sub_type_cd != 'principal_out' then 0 when status_cd IN ('returned','cancelled') then 0 when eff_date BETWEEN funding_date_actual and funding_date_actual + '6 months'::interval then 1 else 0 end) as total_withdraws_6m,
sum(case when sub_type_cd IS NULL then 0 when sub_type_cd != 'principal_out' then 0 when status_cd IN ('returned','cancelled') then 0 when eff_date BETWEEN funding_date_actual and funding_date_actual + '12 months'::interval then 1 else 0 end) as total_withdraws_12m,
sum(case when sub_type_cd IS NULL then 0 when sub_type_cd != 'principal_out' then 0 when status_cd IN ('returned','cancelled') then 0 when eff_date BETWEEN funding_date_actual and funding_date_actual + '1 month'::interval then 1 else 0 end) as total_withdraws_1m,
sum(case when sub_type_cd IS NULL then 0 when sub_type_cd != 'principal_out' then 0 when status_cd IN ('returned','cancelled') then 0 when eff_date BETWEEN funding_date_actual and current_date then 1 else 0 end) as total_withdraws,
sum(case when loan_task_committed_id <= (select loan_task_committed_id from bus_analytics.all_products_cash_flow
where loan_id = td.loan_id
	and (sub_type_cd = 'principal_out')
	and status_cd NOT IN ('returned','cancelled')
	order by loan_task_committed_id
	limit 1) then -1*total_amount else 0 end) as first_withdraw_amount
from temp_bad_rates td
left join bus_analytics.all_products_cash_flow apcf on apcf.loan_id = td.loan_id
--where (sub_type_cd IS NULL OR (sub_type_cd = 'principal_out'))
--and (status_cd IS NULL or status_cd NOT IN ('returned','cancelled'))
group by 1
);

drop table if exists temp_survival_numbers;
create temp table temp_survival_numbers as (
select td.loan_id,
max(case when eff_date > current_date then null when sub_type_cd NOT IN('principal_out','payment','other_credit') then null when status_cd IN ('returned','cancelled') then null else eff_date end ) as last_activity,
max(case when eff_date > current_date then null when sub_type_cd NOT IN('principal_out','payment','other_credit') then NULL when status_cd IN ('returned','cancelled') then NULL else eff_date end ) - td.funding_date_actual as time_alive,
case when max(case when eff_date > current_date then null when sub_type_cd NOT IN('principal_out','payment','other_credit') then NULL when status_cd IN ('returned','cancelled') then NULL else eff_date end ) > current_date - '3 months'::interval then 1 else 0 end as censored
from temp_bad_rates td
left join bus_analytics.all_products_cash_flow apcf on apcf.loan_id = td.loan_id
--where sub_type_cd IN ('principal_out','payment','other_credit')
--and status_cd NOT IN ('returned','cancelled')
--and eff_date < current_date
group by td.loan_id, funding_date_actual
);

drop table if exists temp_payback_stuff;
create temp table temp_payback_stuff as (
select td.loan_id,sum(abs(inn.total_amount)) as in_money, sum(abs(out.total_amount)) as out_money,
	case when sum(abs(inn.total_amount)) >=sum(abs(out.total_amount)) then 1 else 0 end as full_payback_first_payment
from temp_bad_rates td
left join bus_analytics.all_products_cash_flow out on out.loan_id = td.loan_id --get first withdrawn
	and out.sub_type_cd = 'principal_out'
	and out.status_cd NOT IN ('returned','cancelled')
left join bus_analytics.all_products_cash_flow outblock on outblock.loan_id = td.loan_id --make sure is actually first
	and outblock.sub_type_cd = 'principal_out'
	and outblock.status_cd NOT IN ('returned','cancelled')
	and outblock.loan_task_committed_id < out.loan_task_committed_id
left join bus_analytics.all_products_cash_flow inn on inn.loan_id = td.loan_id --get first payment
	and inn.sub_type_cd = 'payment'
	and inn.status_cd NOT IN ('returned','cancelled')
left join bus_analytics.all_products_cash_flow inblock on inblock.loan_id = td.loan_id --make sure is actually first
	and inblock.sub_type_cd = 'payment'
	and inblock.status_cd NOT IN ('returned','cancelled')
	and inblock.loan_task_committed_id < inn.loan_task_committed_id
where outblock.loan_task_committed_id is null
	and inblock.loan_task_committed_id is null
group by 1
);

drop table if exists temp_old_payback_definition;
create temp table temp_old_payback_definition as (
select loan_id
,case when (select avg(nice_div(amount_paid,amount_paid+unpaid_balance)) from bus_analytics.loc_statement_performance where loan_id = td.loan_id and statement_due_date BETWEEN td.funding_date_actual AND td.funding_date_actual + '3 months'::interval)=1 then 1 else 0 end as payback_100
from temp_bad_rates td
);

drop table if exists temp_performance; --note: once Jon finishes his tables add the 12 month stuff here
create temp table temp_performance as (
select td.*,
lp.positive_amount_due_flag,
lp.statement_initial_default_flag,
total_withdrawn_3m,
total_withdrawn_6m,
total_withdrawn_12m,
total_withdrawn,
...
in_money,
out_money,
full_payback_first_payment,
payback_100

,case when td.funding_date_actual <= current_date - interval '9 months' then (select net_fees from mdalgleish.all_customer_net_fees where cnu_customer_id = td.customer_id and month_calculated = 6) else null end as six_month_net_fees
,case when td.funding_date_actual <= current_date - interval '12 months' then (select net_fees from mdalgleish.all_customer_net_fees where cnu_customer_id = td.customer_id and month_calculated = 9) else null end as nine_month_net_fees
,case when td.funding_date_actual <= current_date - interval '15 months' then (select net_fees from mdalgleish.all_customer_net_fees where cnu_customer_id = td.customer_id and month_calculated = 12) else null end as twelve_month_net_fees
,case when td.funding_date_actual <= current_date - interval '18 months' then (select net_fees from mdalgleish.all_customer_net_fees where cnu_customer_id = td.customer_id and month_calculated = 15) else null end as fifteen_month_net_fees
,case when td.funding_date_actual <= current_date - interval '21 months' then (select net_fees from mdalgleish.all_customer_net_fees where cnu_customer_id = td.customer_id and month_calculated = 18) else null end as eighteen_month_net_fees


from temp_bad_rates td

inner join temp_loc_performance lp on lp.loan_id = td.loan_id
inner join temp_withdraw_numbers wn on wn.loan_id = td.loan_id
inner join temp_survival_numbers sn on sn.loan_id = td.loan_id
inner join temp_payback_stuff ps on ps.loan_id = td.loan_id
inner join temp_old_payback_definition opd on opd.loan_id = td.loan_id
);
